...
class Database(object):
    ...
    def post(self, user_id, words, username):
        self.cursor.execute('INSERT INTO post (user_id, words, username) VALUES(?, ?, ?)',
                            (user_id, words, username))
        self.conn.commit()
    ...
    def get_user_info(self, username):
        user = {}
        self.cursor.execute("SELECT user_id, profile_img, first_name, last_name FROM user where username='{}'".format(username))
...
def post():
    ...
    if request.method == 'POST':
        ...
        db.post(user_id, words, username)
        response['status_code'] = 200
